...
public class DiscriminatorWithBatchNorm {
    Convolution conv1;
    ...
    public DiscriminatorWithBatchNorm() {
        int inputWidth = 28, inputHeight = 28;
        this.conv1 = new Convolution(5, 64, 3, inputWidth, inputHeight, 1);
        this.batch1 = new BatchNormalization(conv1.output_depth * conv1.output_height * conv1.output_width);
        ...
        this.batch2 = new BatchNormalization(conv2.output_depth * conv2.output_height * conv2.output_width);
    ...
    public double[] getOutput(double[][] img) {
        ...
        double[][][] output_leakyRELU1 = this.leakyReLULayer1.forward(output_batch1_unflattened);
        ...
        double[][][] output_leakyRELU2 = this.leakyReLULayer2.forward(output_batch2_unflattened);
...
//        double[][][] disc_in_batch1_unflattened = UTIL.unflatten(disc_in_batch1, conv1.output_depth, conv1.output_height, conv1.output_width );
    ...
    public void updateParameters(double[] outputGradient, double learning_rate_disc) {
        ...
        double[] disc_in_batch2 = this.batch2.backward(disc_in_gradient_leakyrelu2_flattened);
        double[][][] disc_in_batch2_unflattened = UTIL.unflatten(disc_in_batch2, conv2.output_depth, conv2.output_height, conv2.output_width);
        double[][][] disc_in_gradient_conv2 = this.conv2.backprop(disc_in_batch2_unflattened);

        double[][][] disc_in_gradient_leakyrelu1 = this.leakyReLULayer1.backward(disc_in_gradient_conv2);
        double[] disc_in_gradient_leakyrelu1_flattened = UTIL.flatten(disc_in_gradient_leakyrelu1);
        double[] disc_in_batch1 = this.batch1.backward(disc_in_gradient_leakyrelu1_flattened);
        double[][][] disc_in_batch1_unflattened = UTIL.unflatten(disc_in_batch1, conv1.output_depth, conv1.output_height, conv1.output_width );
        double[][][] disc_in_gradient_conv1 = this.conv1.backprop(disc_in_batch1_unflattened);

        conv1.updateParameters(disc_in_batch1_unflattened, learning_rate_disc);
        conv2.updateParameters(disc_in_batch2_unflattened, learning_rate_disc);
