...
public class Critic_GP implements Serializable {
    ...
    public Critic_GP(int batchSize, OptimizerHyperparameters optimizerHyperparameters) {
...
//            double[][][] disc_in_gradient_conv1 = this.conv1.backprop(disc_in_gradients_leakyrelu1[i]);
    ...
    public void updateParametersBatch(double[][] outputGradients, double[][][][] dGPdInterpolated) {
        // given the output gradients for the interpolated image(which is just 1 here) and the gradient of GP wrt (dou D(x_hat)/dou x_hat), we update the parameters
        double[][][][] disc_in_gradients_leakyrelu2_conv2_out_grad = new double[batchSize][][][];
        double[][][][] disc_in_gradients_leakyrelu1_conv1_out_grad = new double[batchSize][][][];
        double[][] disc_in_gradients_dense = new double[batchSize][];

        for (int i = 0; i < batchSize; i++) {
            disc_in_gradients_dense[i] = this.dense.backward(outputGradients[i]);

            double[][][] disc_in_gradient_dense_unflattened = MiscUtils.unflatten(disc_in_gradients_dense[i], leakyReLULayer2.output.length, leakyReLULayer2.output[0].length, leakyReLULayer2.output[0][0].length);
            disc_in_gradients_leakyrelu2_conv2_out_grad[i] = this.leakyReLULayer2.backward(disc_in_gradient_dense_unflattened, outputs_leakyRELU2[i]);
...
//        double[][][] y = conv1.backward2_and_update(dGPdInterpolated, disc_in_gradients_leakyrelu1_conv1_out_grad, inputs);
//        y = conv2.backward2_and_update(y, disc_in_gradients_leakyrelu2_conv2_out_grad, outputs_leakyRELU1);
//        y = dense.backward2_and_update(y, outputGradients, outputs_leakyRELU2_flattened);
    }
}